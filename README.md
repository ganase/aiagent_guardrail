...
3. GUI ウィザードでインストール先が自分の `C:\Users\<ユーザー名>\CodingAgentForIT\` になっていることを確認し、「インストール実行」をクリックする（Node.js・Python・Claude Code・Codex が未導入の場合は自動導入するか選択できる）

`SETUP.bat` は別の管理者アカウントへ自動昇格しません。別アカウントの資格情報で実行すると、そのアカウントの認証設定やショートカットが作成されてしまうためです。ACL 保護など管理者権限が必要な導入は、対象者自身が管理者である環境で実行してください。

デフォルトのインストール先: `C:\Users\<ユーザー名>\CodingAgentForIT\`
...
- ワークスペースランチャーで Codex または Claude Code を起動すると、操作履歴・セッションログが Box の個人用 `Sandbox\<ユーザー>\CodingAgentForIT-Audit` に同期されます。認証情報と設定は同期対象外です。
